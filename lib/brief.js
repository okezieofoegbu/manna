...
//
// v0.1.5: each brief_items row now carries a prompt_version column
// recording which synthesis prompt produced it. Useful for the
// observation-hold tracking pattern.
...
export async function readBriefForDate(date) {
  ...
  const { data, error } = await supabase
    .from('brief_items')
    .select(
      'id, date, source_email_id, source_link, sender, subject, category, synthesis, suggested_owner, time_estimate, state, system_flag, prompt_version, created_at',
    )
    .eq('date', date)
