...
//
// v0.1.5: each brief_items row carries prompt_version recording which
// synthesis prompt produced it.
//
// v0.1.5.1: each brief_items row also carries body_excerpt — the same
// raw email body text we already pass to the synthesis prompt. This
// makes the brief self-contained for triage; the user no longer needs
// to click through to Zoho to read the email content. The excerpt is
// stored at write time using the same source (m.summary || m.snippet
// || m.shortContent) and the same truncation cap (800 chars) as the
// synthesis input, so the displayed excerpt matches what the synthesis
// saw. The render layer clamps the visual height via CSS.
//
// v0.1.6: brief_items is now shared with the Vitalis pipeline
// (lib/brief-vitalis.js). Two surgical changes:
//   - readBriefForDate filters by source='zoho_transworld' so it
//     returns only Transworld rows, not the new Vitalis rows.
//   - writeBriefItems sets source='zoho_transworld' explicitly on
//     every row (the DB has a default, but explicit is clearer and
//     avoids surprises if defaults ever change).
...
async function scanInvestmentBreach(accessToken, accountId, folderIds) {
  ...
  for (const m of candidates) {
    ...
    try {
      ...
      // v0.1.4.2 fix: tag _folderId so buildZohoMessageLink can construct
      // a working webmail URL for breach items.
      out.push({
        ...m,
        _folderId: m._folderId || inboxFolder.folderId,
        _threadError: String(e).slice(0, 200),
    ...
    if (!hasReply) {
      // v0.1.4.2 fix: tag _folderId so buildZohoMessageLink can construct
      // a working webmail URL for breach items. (In v0.1.4 the breach
      // path returned messages without _folderId and the page rendered
      // them without source links.)
      out.push({
        ...m,
        _folderId: m._folderId || inboxFolder.folderId,
        _ageHours: Math.round(ageHours),
...
async function writeBriefItems(date, items, flaggedMessages) {
  ...
  const rows = items
    ...
    .map((it) => {
      ...
      // v0.1.5.1 — capture body_excerpt at write time from the same
      // source (and truncation cap) the synthesis prompt saw, so the
      // page can render what the email actually said without a Zoho
      // round-trip.
      const bodyExcerpt = flagged
        ? truncateBody(
            flagged.summary || flagged.snippet || flagged.shortContent || '',
          )
        : '';
      return {
        date,
        // v0.1.6 — explicit source column. brief_items is shared with
        // the Vitalis pipeline; defaults exist but explicit is clearer.
        source: ZOHO_TRANSWORLD_SOURCE,
        ...
        // v0.1.5 — record which synthesis prompt version produced this
        // row, so the historical record carries forward as the prompt
        // evolves through observation holds.
...
function buildZohoMessageLink(flagged) {
  // Zoho Mail webmail URL. NOTE — v0.1.5.1 finding: Zoho's hash-route
  // URLs do NOT deep-link to a specific message. This URL opens the
  // user's Zoho inbox, not the email. We still build it (so future
  // Zoho UI changes might honor it), and the page labels the click-
  // through as "Open in Zoho" rather than pretending it's a per-email
  // link. The user searches by subject once landed in Zoho.
