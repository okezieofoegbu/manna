...
async function runSynthesis(flaggedMessages) {
  ...
  try {
    ...
    throw new Error(
      `Synthesis returned non-JSON output. First 400 chars: ${cleaned.slice(0, 400)}`,
    );
  }
  if (!Array.isArray(parsed)) {
    throw new Error(
      `Synthesis returned non-array. First 400 chars: ${cleaned.slice(0, 400)}`,
    );
  }
  return parsed;
}
...
// ─── token refresh exported through zoho.js ───────────────────────
// (We don't import refreshAccessToken directly because zoho.js keeps
// it module-private; here we wrap.)
// In zoho.js we expose it as refreshAccessTokenInternal for use here.
