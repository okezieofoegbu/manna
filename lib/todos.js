...
// v0.1.8.1 — setDueDate rejects years outside 2020-2100 (safety net for
//          the browser native date picker accepting partial-year typos).
//
...
// ---------------------------------------------------------------------------
// setDueDate — v0.1.8 / sanity bounds added v0.1.8.1
// ---------------------------------------------------------------------------
//
// Set or clear the due_date on a todo. dueDate is a YYYY-MM-DD string
// (a calendar date, no time component) or null to clear.
//
// v0.1.8.1: reject year outside 2020-2100. The browser's native date
// picker on Mac Chrome lets a typo through (e.g. year 0023 when the
// user tabbed off mid-typing) — the client-side min/max attributes
// catch most of these, this is the server-side safety net.
export async function setDueDate(todoId, dueDate) {
  ...
  if (dueDate !== null && dueDate !== undefined && dueDate !== '') {
    ...
    if (year < 2020 || year > 2100) {
      throw new Error(
        `setDueDate: year '${year}' outside reasonable bounds (2020-2100)`,
      );
    }
    value = dueDate;
  }
