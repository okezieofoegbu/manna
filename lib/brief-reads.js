// lib/brief-reads.js
//
// Owner-only read of today's brief(s), used by the server component
// in app/page.js.
//
// Security model:
//   - The page's server component calls getCurrentUser() and checks
//     user.role === 'owner' BEFORE calling these read functions.
//   - These functions additionally take the user object and assert
//     the role themselves, as defence in depth.
//   - All reads go through the service-role client; the anon key
//     can see nothing in brief_items (RLS on, no policies).
//   - When user.role !== 'owner', these return [] and the page
//     renders nothing — no placeholder, no hint that data exists.
//
// v0.1.6 — second source. The Vitalis read is parallel to the
// Transworld one; both are owner-only with the same defence-in-depth
// pattern. groupByCategory is shared across both.

import { readBriefForDate } from './brief.js';
import { readVitalisBriefForDate } from './brief-vitalis.js';
...
// v0.1.6 — Vitalis read. Same owner-only contract, same date helper.
export async function readTodaysVitalisBriefForOwner(user) {
  if (!user || user.role !== 'owner') return [];
  const date = todayOwnerLocal();
  return readVitalisBriefForDate(date);
}
...
// re-sorting. Used by both Transworld and Vitalis sections.
